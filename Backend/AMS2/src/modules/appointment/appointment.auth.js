...
export const authSlots = () => {
    return async (req, res, next) => {
        ...
        for (const staffService of staffsServices) {
            const { staffId, services } = staffService;
            // Here, each "services" item has { serviceId, ... } in your original code
            // We only need the IDs to validate them
            ...
            const validationResult = await validateStaffServices({
                ...
                missingServices,
                unauthorizedServices,
                lean: false, // we can keep this false if we want mongoose documents
            });
            ...
            const { staff, serviceObjects } = validationResult;
            validatedStaffsServices.push({ staff, services: serviceObjects });
        }

        // Check for errors
        if (
            missingStaffs.length > 0 ||
            missingServices.length > 0 ||
            unauthorizedStaffs.length > 0 ||
            unauthorizedServices.length > 0
        ) {
